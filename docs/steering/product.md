...
- **ドラッグ&ドロップ操作**: 直感的なUI操作でタブの並び替えや階層変更が可能。ドロップインジケーターによる視覚的フィードバック、depth選択機能（マウス左右移動で階層変更）、ドラッグ中のタブ位置固定、ツリービュー外へのドロップで新規ウィンドウ作成（サブツリーごと移動）、クロスウィンドウドラッグによるウィンドウ間タブ移動、空ウィンドウの自動クローズ
