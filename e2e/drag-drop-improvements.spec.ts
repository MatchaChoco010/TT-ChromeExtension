...
test.describe('ドラッグ&ドロップ改善', () => {
  ...
  test.describe('ドラッグ中のタブ位置固定', () => {
    ...
    test('ホバー後にドロップすると親子関係が形成されること', async ({
      extensionContext,
      serviceWorker,
    }) => {
      const windowId = await getCurrentWindowId(serviceWorker);
  ...
  test.describe('ドラッグ中の横スクロール防止', () => {
    test('ドラッグ&ドロップが正常に動作すること', async ({
      extensionContext,
      serviceWorker,
    }) => {
      const windowId = await getCurrentWindowId(serviceWorker);
    ...
    test('ドラッグ中にマウスを左右に動かしてもビューが横スクロールしないこと', async ({
      ...
      await sidePanelPage.mouse.move(20, tab2Box.y + tab2Box.height + 10, { steps: 1 });
    ...
    test('ドラッグ終了後にタブ構造が維持されること', async ({
      extensionContext,
      serviceWorker,
    }) => {
      const windowId = await getCurrentWindowId(serviceWorker);
