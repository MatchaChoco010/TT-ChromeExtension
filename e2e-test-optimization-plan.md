...
### 重要な知見: context.close()の遅延

テストに仕込んだログ全体の合計よりテスト自体の実行時間が長い場合は、テストの終了処理のcontext.close()に時間がかかっているはずです。
`context.close()`が遅い場合、**どこかに完了しない待機が意図せず含まれている**。

この待機を特定して修正する必要がある。過去の例として`page.click()`後の`noWaitAfter`や`force`オプションで解決したケースがあるが、これは一例に過ぎない。本質は「余計な待機が残っている」ことが原因であり、それを見つけて解消すること。
テスト終了時にもうその条件を満たすことが不可能な待機が存在するのが問題です。
noWaitAfterをつけないと遅くなるのは、クリック後にその要素が消えるようなコンテキストメニューのような場合に、クリック後の要素が安定するまで待機しようにも要素が消えてしまうのが原因です。
そのような完了を待機することが不可能な条件での待機が残っていないかを調査し、解消すること。
...
- 完了しない待機（context.close()遅延の原因）
- マウス操作ステップの冗長性

根本原因を特定し、完了条件を満たすまで調査・修正を継続する。

### 常に読み込むドキュメント
定期的にe2e-test-optimization-plan.mdを読み直すことをタスクに追加する。
